...
switch lower(FEATURE)    
    ...
    case 'gist'
        ...
        train_image_feats1 = get_gist_feats(train_image_paths);
        test_image_feats1 = get_gist_feats(test_image_paths);
        
        train_image_feats = [train_image_feats,train_image_feats1];
        test_image_feats = [test_image_feats,test_image_feats1];

    case 'fisher'
        if ~exist('vocab_fisher_20.mat', 'file')
            fprintf('No existing visual word vocabulary found. Computing one from training images\n')
            clusters = 20; %Larger values will work better (to a point) but be slower to compute
            [means,covariances,priors] = build_gmm_vocabulary(train_image_paths, clusters);
            save('vocab_fisher_20.mat', 'means','covariances','priors')
        end

        train_image_feats = get_fisher_feats(train_image_paths);
        test_image_feats = get_fisher_feats(test_image_paths);
...
switch lower(CLASSIFIER)    
    ...
    case 'rbf svm'
        predicted_categories = rbf_svm_classify(train_image_feats, train_labels, test_image_feats);
